...
public class RobotControlState {
    ...
    /**
     * Returns the alliance-specific zone identifier based on the current alliance and zone.
     *
     ...
     * Determines the scoring zone based on the robot's position relative to the reef,
     * including both circular distance from the reef and checks within triangle zones.
     * Process:
     * - Ensures alliance and reef information are set.
     * - Computes the distance from the robot to the reef.
     * - If the robot is within a certain distance (AUTO_DRIVE_DISTANCE) from the reef:
     * calculates the angle from the reef to the robot to determine which circular zone
     * the robot is in. Each circular zone is a 60-degree slice around the reef.
     * - Regardless of distance, also checks if the robot is inside any of the special
     * triangular zones near the reef using point-in-triangle math.
     * The math:
     * - Uses the distance formula (from the difference vector's norm) to check proximity.
     * - Uses angle calculations and sectoring (dividing 360° by 60° segments) to find zones.
     * - For triangles, relies on area comparisons to determine if the robot's position
    ...
    /**
     * Calculates the robot's angle relative to the reef, normalized to a 0-360 degree range,
     * and then adjusts it with an offset to align with scoring zones.
     * Process:
     * - First, computes the angle in radians between the horizontal axis and the vector from reef to robot.
     ...
     * Why do this?
     * - This normalized angle is used to determine which of the six equal 60-degree zones
     ...
     * Process:
     * - Depending on the alliance, uses an array of zones that correspond to 60-degree slices of a circle.
     * - Divides the full 360 degrees by 60 to get one of six segments (0 through 5).
     * - Selects the scoring zone corresponding to that segment from the array.
     * The math:
     * - Uses floor division of the angle by 60 to map continuous angle to discrete zone segments.
     * For example, if angleDeg is 45°, then floor(45/60)=0, which corresponds to the first zone in the list.
     *
     * @param angleDeg
    ...
    private static ScoringZone determineZone(double angleDeg, Alliance alliance) {
        ScoringZone[] zones = (alliance == Alliance.Red)
                ? new ScoringZone[] { ScoringZone.ZONE_CD, ScoringZone.ZONE_AB, ScoringZone.ZONE_KL, ScoringZone.ZONE_IJ, ScoringZone.ZONE_GH, ScoringZone.ZONE_EF }
                : new ScoringZone[] { ScoringZone.ZONE_IJ, ScoringZone.ZONE_GH, ScoringZone.ZONE_EF, ScoringZone.ZONE_CD, ScoringZone.ZONE_AB, ScoringZone.ZONE_KL };
    ...
    /**
     * Checks if the robot's position falls within any of the predefined triangular scoring zones.
     * Process:
     * - Depending on the alliance (Red or Blue), selects the corresponding set of triangle vertices.
     * - For each triangle zone, retrieves its three corner points.
     * - Uses the pointInTriangle method to check if the robot lies inside the triangle.
     * The math:
     * - It calculates the area of the triangle formed by the triangle’s vertices.
     * - Then calculates areas of three smaller triangles formed by replacing one vertex
    ...
    /**
     * Checks if a given point p lies inside the triangle defined by points a, b, and c.
     * Math explanation:
     ...
     * (within a small tolerance to account for floating point precision), then point p
     * lies inside the triangle.
     * Why does this work?
     * - If p is inside triangle abc, then the smaller triangles cover the exact area of abc.
    ...
    private static boolean pointInTriangle(Translation2d p, Translation2d a, Translation2d b, Translation2d c) {
        ...
        // The point is inside the triangle if the areas add up (within a small tolerance).
        return Math.abs(area - (area1 + area2 + area3)) < 1e-6;
    }

    /**
     * Computes the absolute area of the triangle defined by points p1, p2, and p3 using the
     * shoelace formula.
     * Math explanation:
     * - Given three points (x1, y1), (x2, y2), and (x3, y3),
     * the area is |x1(y2 - y3) + x2(y3 - y1) + x3(y1 - y2)| / 2.
     * - This formula effectively calculates a determinant that gives twice the area,
